...
class Logic():
    ...
    logic_and = [[np.array([0,0]), 0], [np.array([0,1]), 0], [np.array([1,0]) ,0], [np.array([1,1]), 1]]
    logic_or = [[np.array([0,0]), np.array([1, 0])], [np.array([0,1]), np.array([0, 1])], [np.array([1,0]) ,np.array([0, 1])], [np.array([1,1]), np.array([0, 1])]]
    logic_exor = [[np.array([0,0]), [1, 0]], [np.array([0,1]), [0, 1]], [np.array([1,0]) ,[0, 1]], [np.array([1,1]), [1, 0]]]
...
if __name__ == '__main__':
    ...
    ## LOGIC dataset
        ...
        logic_and_test  = [(x, np.argmax(y)) for x, y in data.logic_and]
        #logic_or_test  = np.array([(x, np.argmax(y)) for x, y in data.logic_or])
        logic_or_test  = [(x, np.argmax(y)) for x, y in data.logic_or]
        ...
        net.SGD(train_data, 10, 2, 0.9, test_data=test_data)
